...
| ⬜ Open | P1 | **Debug `console.log` left in production storage code**: `getConversationMessages` in messaging storage has 3 active `console.log` calls (lines 243, 250, 273) that fire on every conversation open. Can expose contact phone/email data in server logs. Remove or gate behind a debug flag. | `server/storage/messaging.ts` |
...
| ⬜ Open | P1 | **51 `req.user!` non-null assertions** remain in route files not covered by the prior audit: `contact-actions.ts` (11), `hcp-scheduling.ts` (9), `ai.ts` (9), `housecall-pro.ts` integrations (8), `google-sheets.ts` (6), `oauth.ts` (3), `auth.ts` (3), `employees.ts` (2). Migrate all to `getAuthUser(req, res)` helper. | `server/routes/contact-actions.ts`, `server/routes/integrations/hcp-scheduling.ts`, `server/routes/ai.ts`, `server/routes/integrations/housecall-pro.ts`, `server/routes/integrations/google-sheets.ts`, `server/routes/oauth.ts`, `server/routes/auth.ts`, `server/routes/employees.ts` |
...
| ⬜ Open | P1 | **`findMatchingContact` still uses `REGEXP_REPLACE`**: Called on every webhook inbound contact-match, this function (contacts.ts ~line 431) still runs a raw SQL `REGEXP_REPLACE` full-table scan. Fix: rewrite to use the `normalizedPhone` indexed column (same approach as `getContactByPhone`). | `server/storage/contacts.ts` |
| ⬜ Open | P2 | **Notification polling redundant with WebSocket**: `NotificationDropdown.tsx` polls `/api/notifications` every 60 seconds via `refetchInterval`. At 10x load (100 users), this is ~100 DB queries/min from polling alone. WebSocket events already drive badge updates. Remove the interval and rely solely on WS invalidation + fetch-on-open. | `client/src/components/NotificationDropdown.tsx` |
| ⬜ Open | P2 | **`getConversationMessages` sequential queries**: Runs 3 DB queries in series (SMS, email activities, call activities). Parallelize with `Promise.all` to cut latency by ~2/3. | `server/storage/messaging.ts` |
...
| Status | Priority | Description | File(s) |
|--------|----------|-------------|---------|
| ✅ Added | P1 | `req.user!` used 300+ times across route files. Added `getAuthUser(req, res)` helper. Core route files migrated. | All `server/routes/*.ts` |
...
| ⬜ Open | P2 | **`INTEGRATION_NAMES.includes(integrationName as any)`** appears 7 times (6 in `index.ts`, 1 in `provider-service.ts`). Replace with an `isIntegrationName(v: string): v is IntegrationName` type guard to eliminate all casts at once. | `server/routes/integrations/index.ts`, `server/providers/provider-service.ts` |
...
| ⬜ Open | P2 | **Bulk mutations missing WebSocket broadcast**: Bulk status update and bulk delete mutations invalidate local cache but don't emit `broadcastToContractor` events. Other open user sessions won't see changes until manual refresh. | `server/routes/contacts.ts`, `client/src/hooks/useBulkActions.ts` |
| ⬜ Open | P2 | **HCP main webhook missing broadcasts**: `housecall-pro.ts` triggers workflow events for `estimate.updated`, `job.created/updated`, and `customer.created/updated` but has zero `broadcastToContractor` calls. The dedicated `webhooks/jobs.ts` and `webhooks/estimates.ts` files do broadcast, but this catch-all handler does not. | `server/routes/webhooks/housecall-pro.ts` |
| ⬜ Open | P2 | **`Follow-ups.tsx` missing status-count invalidation**: `updateLeadFollowUpMutation` does not invalidate `/api/contacts/status-counts`. If a follow-up date change also changes lead status, count badges won't update. | `client/src/pages/Follow-ups.tsx` |
