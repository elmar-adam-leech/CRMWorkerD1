...
| Status | Priority | Description | File(s) |
|--------|----------|-------------|---------|
| ✅ Fixed | P0 | JWT is returned in the JSON response body from `/api/auth/login` and `/api/auth/register` in addition to the httpOnly cookie. Removed `token` field from both responses — browser SPA uses the httpOnly cookie; Bearer header clients re-authenticate on startup. | `server/routes/auth.ts` |
| ✅ Fixed | P0 | No token revocation on logout — a captured token stayed valid up to 7 days after logout. Added `revoked_tokens` table + `jti` claim in JWT. Logout inserts jti; `requireAuth` queries the table on every request. Hourly cleanup in `server/index.ts`. | `server/auth-service.ts`, `server/routes/auth.ts`, `server/index.ts`, `shared/schema.ts` |
| ✅ Fixed | P1 | No "sign out all devices" protection — stolen phone could not be invalidated remotely. Added `tokenVersion` column on `users` + `POST /api/auth/logout-all`. Increments tokenVersion invalidating all prior tokens. SecurityTab UI added. | `shared/schema.ts`, `server/auth-service.ts`, `server/routes/auth.ts`, `client/src/components/settings/SecurityTab.tsx` |
