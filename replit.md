...
- `asyncHandler` (`async-handler.ts`) — HOF wrapping `async (req, res, next)` route handlers: catches any thrown error and forwards it to `next(error)`. Eliminates per-handler `try { ... } catch { res.status(500) }` boilerplate. Applied to all simple-500-catch handlers across: `contacts.ts`, `jobs-estimates.ts`, `messaging.ts`, `users.ts`, `auth.ts`, `workflows.ts` (all 23 handlers), `integrations/dialpad.ts` (all 13 handlers), `integrations/google-sheets.ts` (all 6 outer handlers; 2 inner try/catch blocks for sub-operation errors preserved), `integrations/housecall-pro.ts` (all 16 outer handlers; 2 inner try/catch blocks inside sync loop preserved). A global 4-argument error handler in `server/routes.ts` receives forwarded errors and returns a structured `{ message }` JSON response with the correct HTTP status.
...
- `useCurrentUser()` — Fetches `/api/auth/me` with a 5-minute stale time, no window-focus refetch. Returns `{ data: CurrentUser | undefined, isLoading, ... }`. The `CurrentUser` interface includes `user` (id, email, role, name), `contractor` (id, name, slug), and `hasActiveCompanyIntegrations?`. All pages/components must use this hook instead of declaring their own raw `useQuery('/api/auth/me')`. Currently used by: `IntegrationsTab.tsx`, `AccountTab.tsx`, `GmailConnectionCard.tsx`, `SalespeopleManagement.tsx`, `Templates.tsx`, `AI-Monitor.tsx`, `Settings.tsx`, `use-sync-status.tsx`, `TargetsTab.tsx`, `NodeEditDialog.tsx`, `WorkflowsList.tsx`.
  - `isAdminUser(role?)` — exported helper, returns `true` for `admin`, `super_admin`, and `manager`. Used where managers need access (AI-Monitor, WorkflowsList, NodeEditDialog).
  - `isStrictAdmin(role?)` — exported helper, returns `true` for `admin` and `super_admin` only (excludes manager). Used where strictly admin access is required (Templates approval, Settings account tab, TargetsTab editing).
- `usePagePreferences({ pageKey })` — Persists `viewMode`, `filterStatus`, `advancedFilters` to localStorage per page. Currently wired into Jobs, Estimates, and Leads.
- `useHcpImport({ entityType, syncStartDate, queryKeysToInvalidate })` — Encapsulates the Housecall Pro import flow: `importDateOpen`/`setImportDateOpen`, `selectedImportDate`/`setSelectedImportDate`, a seeding `useEffect` from `syncStartDate`, and `handleConfirmImport` (optionally updates sync-start-date, POSTs to `/api/housecall-pro/sync?type=...`, invalidates provided query keys, shows toasts). Eliminates ~40 lines of near-identical code previously duplicated in `Jobs.tsx` and `Estimates.tsx`. Usage: `const { importDateOpen, setImportDateOpen, selectedImportDate, setSelectedImportDate, handleConfirmImport } = useHcpImport({ entityType: 'jobs', syncStartDate, queryKeysToInvalidate: [...] })`.
- `useWebSocketInvalidation(rules)` — Subscribes to the WebSocket channel and invalidates TanStack Query keys when matching message types arrive. Accepts `rules: Array<{ types: string[], queryKeys: string[] }>`. Uses a `useRef` for rules to avoid stale closure issues. Eliminates the per-page `subscribe` + `invalidateQueries` `useEffect` previously duplicated in `Jobs.tsx` (~9 lines), `Estimates.tsx` (~13 lines), and `Leads.tsx` (~12 lines).
